...
esp_err_t wifi_prov_event_handler_register(void)
{
    esp_err_t err;

    err = esp_event_handler_register(WIFI_PROV_EVENT, ESP_EVENT_ANY_ID, &wifi_prov_event_handler, NULL);
    if (err != ESP_OK) return err;

#ifdef CONFIG_EXAMPLE_PROV_TRANSPORT_SOFTAP
    err = esp_event_handler_register(WIFI_EVENT, ESP_EVENT_ANY_ID, &wifi_prov_event_handler, NULL);
    if (err != ESP_OK) return err;
#endif

#ifdef CONFIG_EXAMPLE_PROV_TRANSPORT_BLE
    err = esp_event_handler_register(PROTOCOMM_TRANSPORT_BLE_EVENT, ESP_EVENT_ANY_ID, &wifi_prov_event_handler, NULL);
    if (err != ESP_OK) return err;
#endif

    err = esp_event_handler_register(PROTOCOMM_SECURITY_SESSION_EVENT, ESP_EVENT_ANY_ID, &wifi_prov_event_handler, NULL);
    if (err != ESP_OK) return err;
